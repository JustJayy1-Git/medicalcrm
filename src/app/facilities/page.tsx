...
export default async function FacilitiesPage() {
  ...
  return (
    <AppShell user={user} active="/facilities">
            ...
            <tbody className="divide-y divide-stone-200">
              ...
              {(facilities ?? []).map((f) => (
                <tr key={f.id} className="hover:bg-stone-50">
                  <td className="px-4 py-3 text-stone-900 font-medium">{f.name}</td>
                  <td className="px-4 py-3 text-stone-600">{f.city ?? "—"}</td>
                  <td className="px-4 py-3 text-stone-600">{f.state ?? "—"}</td>
                  <td className="px-4 py-3 text-stone-600">{f.phone ?? "—"}</td>
