...
export async function createCase(formData: FormData): Promise<void> {
  ...
  const patient_id = str("patient_id");
  if (!patient_id) redirect("/patients");
  ...
  const payload = {
    ...
    global_coverage_until: str("global_coverage_until"),
    print_patient_statement: bool("print_patient_statement"),
    cash_case: bool("cash_case"),
  ...
  if (error) {
    redirect(
      `/cases/new?patient=${patient_id}&error=${encodeURIComponent(
        error.message,
      )}`,
    );
  }
