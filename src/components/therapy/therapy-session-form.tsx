  ...
  return (
    <section className="lux-card rounded-xl border border-vice-border bg-white p-6 shadow-sm">
      ...
      <form action={addTherapySessionAction} className="space-y-4">
        ...
        <fieldset>
          <legend className="block text-xs font-semibold uppercase tracking-wider text-eggplant-700 mb-2">
            Services performed
          </legend>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {services.map((s) => (
              <label
                key={s.code}
                className="flex items-center gap-3 rounded-lg border border-vice-border bg-vice-surface px-3 py-2.5 text-sm text-eggplant-900 cursor-pointer hover:border-neon-mint transition-colors"
              >
                <input
                  type="checkbox"
                  name="services"
                ...
                <span>
                  {s.label}
                  <span className="text-vice-muted ml-1.5 text-xs">({s.code})</span>
                </span>
              </label>
            ))}
          </div>
