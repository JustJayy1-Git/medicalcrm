-- Massage therapist portal: therapy consent + session sheets (post-NP workflow)
...
-- One consent-for-therapy per case, signed by the patient on the therapist iPad.
...
-- One row per therapy visit (therapy sheet).
