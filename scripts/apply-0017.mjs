/**
 * Apply migration 0017_intake_packet_case_link.sql
 * Run: npm run db:migrate-0017
 */
...
async function main() {
  ...
  const sql = readFileSync(
    resolve(root, "supabase/migrations/0017_intake_packet_case_link.sql"),
    "utf8",
  );

  console.log(`Applying 0017_intake_packet_case_link.sql to ${ref}…`);
  const client = new pg.Client({ connectionString, ssl: { rejectUnauthorized: false } });
  await client.connect();
  try {
    await client.query(sql);
    console.log("OK — intake_packets.case_id added.");
  } finally {
    await client.end();
  }
}
